...
def reports_above_temp(weather,temp):
    ...
    for key in weather.keys():
        if 'temperature' in weather[key]:
            if "F" in weather[key]["temperature"]['units']:
                conv = to_celsius(weather[key]['temperature']['value'])
                if conv > temp:
                    result.append(conv)
            elif "C" in weather[key]["temperature"]['units']:
                if weather[key]['temperature']['value'] > temp:
                    result.append(weather[key]['temperature']['value'])
    return len(result)
